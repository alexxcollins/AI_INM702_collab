...
for position in p:
    num[position] = change_factor(
        test1,
        1000,
        factor={"positions": pos[position]},
        magnitude=outlier_magnitude,
        original_X=X,
        original_y=y,
        original_beta=set1.beta,
    )
...
for position in p:
    mag[position] = change_factor(
        test2,
        1000,
        factor={"magnitude": Magnitude_of_outliers},
...
X_train, X_test, y_train, y_test = train_test_split(
    test4.X, test4.y, test_size=0.2, random_state=42
